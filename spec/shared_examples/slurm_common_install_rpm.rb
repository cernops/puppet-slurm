...
shared_examples_for 'slurm::common::install::rpm' do
  ...
  context 'when version => "19.05.3-2.el7"' do
    ...
    base_packages.each do |p|
      it { should contain_package(p).with_ensure('19.05.3-2.el7').without_require }
    end
  end
  ...
  context 'when repo_baseurl defined' do
    ...
    base_packages.each do |p|
      it { should contain_package(p).with_ensure('present').with_require('Yumrepo[slurm]') }
    end
  end
...
shared_examples_for 'slurm::common::install::rpm-slurmd' do
  it { should contain_package('slurm-slurmd').with_ensure('present').without_require }
end
shared_examples_for 'slurm::common::install::rpm-slurmctld' do
  it { should contain_package('slurm-slurmctld').with_ensure('present').without_require }
end
